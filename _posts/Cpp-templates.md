...
## Quick Power (calculate a^b mod p in O(log n) )

$\color{white}{\text{什么？你问我为什么不用LaTeX？我也想知道为什么标题用不了LaTeX QAQ}}$

```cpp
int quickpower(int a, int b, int p) // calculate $ a^b mod p $
{
	a %= p;
	int ans = 1, base = a;
	while(b){
...
{
#ifndef ONLINE_JUDGE
	freopen(problemname "_8.in", "r", stdin);
